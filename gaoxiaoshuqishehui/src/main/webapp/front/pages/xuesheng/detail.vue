			...
			<view class="detail-content">
				...
				<view class="detail-list-item"
					...
					<view class="lable"
						:style='{"width":"auto","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}'>
						手机：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.shouji}}
					</view>
				</view>
				<view class="detail-list-item"
					:style='{"boxShadow":"0 0 10rpx rgba(0,0,0,.3)","margin":"0 0 10rpx 0","borderColor":"rgba(51, 153, 153, 1)","backgroundColor":"#fff","borderRadius":"10rpx","borderWidth":"2rpx","borderStyle":"solid","height":"68rpx"}'>
...
<script>
	export default {
		...
		methods: {
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discuss学生/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('学生', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('学生', this.detail);
				this.$utils.msg('审核成功');
