        ...
        <view class="container">
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
			    ...
			    <view style="text-align: right;">
					<button @tap="onSubmit(detail.id)" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
			                :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">立即报名</button>
			    </view>
			</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('huodongfabuState')
			if (cleanType) {
				uni.removeStorageSync('huodongfabuState')
				this.mescroll.num = 1
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('huodongfabu', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.huodongfabuPhoto ? this.detail.huodongfabuPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('huodongfabu', {
					page: mescroll.num,
					limit: mescroll.size,
					huodongfabuId: this.id
				});
				// 如果是第一页数据置空
			...
			onSubmit(id){
				uni.redirectTo({
				    url: '/pages/huodongbaoming/add-or-update?id='+id
				});
			},
		}
	}
